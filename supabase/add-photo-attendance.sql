-- 1. Tambah kolom photo URL di tabel attendances
alter table public.attendances 
  add column check_in_photo_url text,
  add column check_out_photo_url text;

-- 2. Buat Storage Bucket untuk foto absensi
insert into storage.buckets (id, name, public)
values ('attendance-photos', 'attendance-photos', true)
on conflict (id) do nothing;

-- 3. Atur RLS Policies untuk Bucket 'attendance-photos'
-- (RLS pada storage.objects secara default sudah aktif di Supabase)

-- Policy: Siswa (Authenticated) bisa upload (INSERT) ke folder dengan nama student_id mereka
...
-- Policy: Semua orang (atau Admin) bisa baca (SELECT) foto absensi
create policy "Bisa baca foto absensi"
...
-- 4. Modifikasi Function perform_attendance_action untuk menerima dan menyimpan photo path
drop function if exists public.perform_attendance_action(text, double precision, double precision, double precision);
...
begin
  ...
  if action_name = 'check_in' then
    ...
    set
      ...
      check_out_photo_url = input_photo_path,
      updated_at = timezone('utc', now())
