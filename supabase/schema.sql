...
begin
  ...
  jenis_label := case when new.request_type = 'leave' then 'Izin' else 'Sakit' end;

  insert into public.notifications (recipient_auth_user_id, title, message, event_type)
  select
    profiles.auth_user_id,
    'Pengajuan ' || jenis_label || ' Baru',
    coalesce(student_name, 'Siswa') || ' mengajukan ' || lower(jenis_label) ||
      ' mulai ' || new.start_date::text || ' s.d. ' || new.end_date::text ||
      '. Alasan: ' || left(btrim(new.reason), 80),
    'leave_request_submit'
  from public.profiles
  where profiles.role = 'admin' and profiles.active = true;
